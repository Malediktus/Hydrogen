...
VulkanShader::VulkanShader(const String& name, const DynamicArray<uint32_t>& vertexSrc, const DynamicArray<uint32_t>& fragmentSrc, const DynamicArray<uint32_t>& geometrySrc)
    ...
    if (vertexSrc.size() > 0) {
        ...
        VK_CHECK_ERROR(vkCreateShaderModule(Renderer::GetContext<VulkanContext>()->GetDevice(), &createInfo, nullptr, &m_VertexShaderModule),
    ...
    if (fragmentSrc.size() > 0) {
        ...
        VK_CHECK_ERROR(vkCreateShaderModule(Renderer::GetContext<VulkanContext>()->GetDevice(), &createInfo, nullptr, &m_FragmentShaderModule),
                       "Failed to create vulkan shader module!");
    ...
    if (geometrySrc.size() > 0) {
        ...
        VK_CHECK_ERROR(vkCreateShaderModule(Renderer::GetContext<VulkanContext>()->GetDevice(), &createInfo, nullptr, &m_GeometryShaderModule),
                       "Failed to create vulkan shader module!");
...
VulkanShader::~VulkanShader() {
    ...
    if (m_VertexShaderModule != VK_NULL_HANDLE)
        vkDestroyShaderModule(Renderer::GetContext<VulkanContext>()->GetDevice(), m_VertexShaderModule, nullptr);
    if (m_FragmentShaderModule != VK_NULL_HANDLE)
        vkDestroyShaderModule(Renderer::GetContext<VulkanContext>()->GetDevice(), m_FragmentShaderModule, nullptr);
    if (m_GeometryShaderModule != VK_NULL_HANDLE)
        vkDestroyShaderModule(Renderer::GetContext<VulkanContext>()->GetDevice(), m_GeometryShaderModule, nullptr);
}
