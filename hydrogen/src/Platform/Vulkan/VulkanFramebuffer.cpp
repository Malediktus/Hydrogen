...
void VulkanFramebuffer::SetDrawBuffers(const DynamicArray<std::pair<FramebufferAttachment, uint32_t>>& attachments) {
    ZoneScoped;
    HY_LOG_TRACE("Set Vulkan draw buffers for framebuffer (ID: {})", m_RendererID);
}

void VulkanFramebuffer::AttachColorTexture(const ReferencePointer<Texture2D>& texture) {
    ZoneScoped;
    HY_ASSERT(numColorAttachments < 32, "Too many color attachments");
    HY_LOG_TRACE("Attached texture of type color buffer to Vulkan framebuffer (ID: {}, TextureID: {})", 0, 0);
}

void VulkanFramebuffer::AttachDepthTexture(const ReferencePointer<Texture2D>& texture) {
    ZoneScoped;
    HY_ASSERT(numDepthAttachments < 1 && numDepthStencilAttachments < 1, "Too many depth attachments");
    HY_LOG_TRACE("Attached texture of type depth buffer to Vulkan framebuffer (ID: {}, TextureID: {})", 0, 0);
}

void VulkanFramebuffer::AttachStencilTexture(const ReferencePointer<Texture2D>& texture) {
    ZoneScoped;
    HY_ASSERT(numStencilAttachments < 1 && numDepthStencilAttachments < 1, "Too many stencil attachments");
    HY_LOG_TRACE("Attached texture of type stencil buffer to Vulkan framebuffer (ID: {}, TextureID: {})", 0, 0);
}

void VulkanFramebuffer::AttachDepthStencilTexture(const ReferencePointer<Texture2D>& texture) {
    ZoneScoped;
