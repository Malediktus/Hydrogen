...
ReferencePointer<Shader> Shader::Create(ReferencePointer<RenderDevice> renderDevice, const String& name, const DynamicArray<uint32_t>& vertexSrc,
                                        const DynamicArray<uint32_t>& fragmentSrc, const DynamicArray<uint32_t>& geometrySrc) {
  ZoneScoped;
  switch (Renderer::GetAPI()) {
    case RendererAPI::API::Vulkan:
      return NewReferencePointer<Vulkan::VulkanShader>(renderDevice, name, vertexSrc, fragmentSrc, geometrySrc);
    default:
...
ReferencePointer<Shader> ShaderLibrary::Load(ReferencePointer<RenderDevice> renderDevice, const String& name, const DynamicArray<uint32_t>& vertexSrc,
                                             const DynamicArray<uint32_t>& fragmentSrc, const DynamicArray<uint32_t>& geometrySrc) {
  ZoneScoped;
  auto shader = Shader::Create(renderDevice, name, vertexSrc, fragmentSrc, geometrySrc);
  Add(name, shader);
  return shader;
}
