...
VulkanShader::VulkanShader(ReferencePointer<RenderDevice> renderDevice, const String& name, const DynamicArray<uint32_t>& vertexSrc, const DynamicArray<uint32_t>& fragmentSrc,
                           const DynamicArray<uint32_t>& geometrySrc)
    : m_RenderDevice(std::dynamic_pointer_cast<VulkanRenderDevice>(renderDevice)), m_Name(name) {
  ZoneScoped;
